...
class TestPlace(unittest.TestCase):
    def setUp(self):
        self.user = User(email="host@example.com", password="password", last_name="User")
        self.city = City(name="Test City", country="Test Country")
        self.place = Place(name="Test Place", descrition="A place for testing", address="123 Test St", city=self.city, host=self.user, number_of_rooms=2, bathrooms=1, price_per_night=100, max_guest=4)


    def test_create_place(self):
        self.assertIsInstance(self.place, Place)
        self.asserEqual(self.place.name, "Test Place")
